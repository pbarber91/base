  ...
  const { data: profile, isLoading: loadingProfile, error: profileError } = useQuery<ProfileRow | null>({
    ...
    queryFn: async () => {
      ...
      if (viewEmail && !isAdmin && !isOwnProfile) {
        // Returning null keeps UI safe; you can also redirect
        return null;
      }
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (data: Record<string, any>) => {
      ...
      const payload: Partial<ProfileRow> & { id: string } = {
        id: targetId,
        // keep email stable when editing own; admin editing someone else should not overwrite email
        ...(isOwnProfile ? { email: authUser.email ?? null } : {}),
        display_name: data.display_name ?? null,
        bio: data.bio ?? null,
        church_id: data.church_id ? String(data.church_id) : null,
        ...
        <div className="max-w-5xl mx-auto px-6 py-12">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            <div className="relative">
              <UserAvatar name={profile?.display_name || authUser?.email} imageUrl={profile?.avatar_url} size="xl" />
