...
// Strict-ish UUID v4/v1… validator (enough to avoid PostgREST uuid parse errors)
const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

function toUuidList(values: any[]): string[] {
  return (values ?? [])
...
export default function Studies() {
  ...
  // 2) Membership lookup:
  //    - group_members.group_id (if you have it)
  //    - church_members.church_id (if you have it)
  //    - PLUS: profiles.church_id (since you *are* storing that and it’s your “active church” right now)
  const membershipsQ = useQuery({
    queryKey: ["study-memberships", user?.id],
    enabled: !!user?.id && !loading,
    queryFn: async (): Promise<{ groupIds: string[]; churchIds: string[] }> => {
      const [gm, cm, prof] = await Promise.all([
        supabase.from("group_members").select("group_id").eq("user_id", user!.id),
        supabase.from("church_members").select("church_id").eq("user_id", user!.id),
        supabase.from("profiles").select("church_id").eq("id", user!.id).maybeSingle(),
      ]);

      // If a table doesn’t exist in a given environment, Supabase returns an error.
      // We treat “missing table” as “no memberships” instead of crashing the page.
      const groupIds = gm.error ? [] : toUuidList((gm.data ?? []).map((r: any) => r.group_id));
      const churchIdsFromMembers = cm.error ? [] : toUuidList((cm.data ?? []).map((r: any) => r.church_id));

      const churchIdFromProfile =
        !prof.error && prof.data?.church_id && UUID_RE.test(String(prof.data.church_id))
          ? [String(prof.data.church_id)]
          : [];

      // Merge + dedupe
      const churchIds = Array.from(new Set([...churchIdsFromMembers, ...churchIdFromProfile]));

      return { groupIds: Array.from(new Set(groupIds)), churchIds };
    },
    staleTime: 30_000,
  });
  ...
  const sharedSessionsQ = useQuery({
    ...
    queryFn: async (): Promise<StudySessionRow[]> => {
      ...
      // IMPORTANT: if these lists contain "" or invalid strings, PostgREST will throw:
      // "invalid input syntax for type uuid: """
      const safeGroupIds = toUuidList(groupIds);
      const safeChurchIds = toUuidList(churchIds);

      if (safeGroupIds.length === 0 && safeChurchIds.length === 0) return [];
      ...
      if (safeGroupIds.length > 0) orParts.push(`group_id.in.(${safeGroupIds.join(",")})`);
      if (safeChurchIds.length > 0) orParts.push(`church_id.in.(${safeChurchIds.join(",")})`);
  ...
  const deleteSessionMutation = useMutation({
    mutationFn: async (sessionId: string) => {
      ...
      // Safety: only allow deleting your own sessions from this page
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["my-study-sessions"] });
      await queryClient.invalidateQueries({ queryKey: ["shared-study-sessions"] });
    },
  });
  ...
  const confirmDelete = (id: string) => {
    const ok = window.confirm("Delete this study session? This will remove your saved progress for it.");
    if (!ok) return;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-14">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            <p className="text-lg text-amber-100 leading-relaxed">
              A Study is a guided walk through a passage using fixed prompts. You pick a reference and a track
              (Beginner / Intermediate / Advanced), then work through the questions and save your progress.
            </p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10 space-y-10">
                ...
                <motion.div key={s.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                  <GradientCard className="p-5 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between gap-3">
                      <button onClick={() => openSession(s.id)} className="min-w-0 flex-1 text-left" aria-label={`Resume ${getReference(s)}`}>
        ...
        {/* Errors */}
        {(mySessionsQ.isError || membershipsQ.isError || sharedSessionsQ.isError || deleteSessionMutation.isError) && (
