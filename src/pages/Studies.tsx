...
function isUuid(v: string) {
  // strict-ish UUID v4-ish format (works fine for all UUIDs generated by Postgres)
...
export default function Studies() {
  ...
  const membershipsQ = useQuery({
    ...
    queryFn: async (): Promise<{ groupIds: string[]; churchIds: string[] }> => {
      ...
      const groupIds = (gm.data ?? [])
        .map((r: any) => String(r.group_id ?? "").trim())
        .filter((v: string) => v.length > 0 && isUuid(v));

      const churchIds = (cm.data ?? [])
        .map((r: any) => String(r.church_id ?? "").trim())
        .filter((v: string) => v.length > 0 && isUuid(v));
  ...
  // 3) Shared sessions (safe querying: avoid PostgREST `or(group_id.in.(...))` string building)
  const sharedSessionsQ = useQuery({
    queryKey: [
      "shared-study-sessions",
      user?.id,
      (membershipsQ.data?.groupIds ?? []).join(","),
      (membershipsQ.data?.churchIds ?? []).join(","),
    ],
    ...
    queryFn: async (): Promise<StudySessionRow[]> => {
      ...
      const all = results.flatMap((r) => (r as any).data ?? []) as StudySessionRow[];

      // Dedup by id
      const seen = new Set<string>();
      const deduped: StudySessionRow[] = [];
      ...
      // Sort: active first, then updated_at desc
        ...
        <div className="max-w-7xl mx-auto px-6 py-14">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
            ...
            <p className="text-lg text-amber-100 leading-relaxed">
              A Study is a guided walk through a passage using fixed prompts. You pick a reference and a track (Beginner / Intermediate /
              Advanced), then work through the questions and save your progress.
            </p>
