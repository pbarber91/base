...
function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  // While auth/profile is resolving, don't redirect.
  if (loading) return <>{children}</>;

  // Not logged in => no onboarding gating.
  if (!user) return <>{children}</>;

  // If profile hasn't loaded yet, don't redirect (prevents flash loops).
  // (AuthProvider fetchProfile is async; this keeps UX stable.)
  if (!profile) return <>{children}</>;

  const completedAt = (profile as any)?.profile_completed_at ?? null;
  const isComplete = !!completedAt;

  // Allow onboarding/auth routes even if incomplete
  ...
  if (!isComplete && !allowlist.has(location.pathname)) {
